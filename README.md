...
- **绝对路径设备无关化**：`settings.json` 和 hook/状态栏脚本里的 `C:/Users/<用户名>/.claude` 在 push 时替换为 `__CLAUDE_DIR__`，pull 时还原为当前设备自己的 `~/.claude`——hooks、toast 通知和底部状态栏因此跨用户名/盘符可用。请勿把写死的绝对路径提交进仓库。
